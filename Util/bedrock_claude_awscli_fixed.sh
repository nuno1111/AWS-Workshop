...
# 기본 설정
DEFAULT_PROMPT="안녕하세요! AWS Bedrock에서 실행되고 있는 Claude 모델입니다. 간단한 자기소개를 해주세요."
...
generate_high_cost_prompt() {
    ...
    case "$model_name" in
        "claude-3-haiku")
            base_prompt="다음 주제들에 대해 각각 매우 상세하고 포괄적인 설명을 작성해주세요. 각 주제마다 최소 10000단어 이상으로 작성하고, 구체적인 예시, 코드 샘플, 실제 사례를 포함해주세요. 가능한 한 길고 자세하게 작성해주세요:
...
각 주제에 대해 초보자도 이해할 수 있도록 단계별로 설명하고, 실무에서 활용할 수 있는 팁과 베스트 프랙티스를 포함해주세요."
        ...
        "claude-3.5-sonnet-v2"|"claude-3.7-sonnet"|"claude-sonnet-4")
            base_prompt="다음 주제들에 대해 매우 상세하고 전문적인 분석을 작성해주세요. 각 주제마다 최소 3000단어 이상으로 작성하고, 가능한 한 길고 자세하게 설명해주세요. 절대로 짧게 요약하지 말고, 모든 세부사항을 포함해서 작성해주세요:
