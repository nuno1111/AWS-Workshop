...
중요: 가능한 한 길고 자세하게 작성해주세요. 각 섹션마다 최소 5000단어 이상으로 작성하고, 모든 세부사항을 포함해주세요. 짧게 요약하지 말고, 예제와 설명을 풍부하게 포함하여 매우 포괄적으로 작성해주세요. 더 많은 내용을 포함할수록 좋습니다. 절대로 간단히 끝내지 말고, 계속해서 더 많은 정보와 예제를 제공해주세요. 

추가 요구사항: 각 주제마다 구체적인 코드 예제를 최소 10개 이상 포함하고, 실제 기업 사례를 5개 이상 분석하며, 관련 도구와 라이브러리를 20개 이상 소개해주세요. 모든 내용을 매우 상세하게 설명하고, 초보자부터 전문가까지 모든 수준에서 이해할 수 있도록 단계별로 작성해주세요."
...
get_max_tokens() {
    ...
    if [[ "$use_high_cost" == "true" ]]; then
        case "$model_name" in
            ...
            "claude-3.5-sonnet-v2"|"claude-3.7-sonnet"|"claude-sonnet-4") echo "100000" ;;  # sonnet도 최대 토큰으로 설정
            *) echo "100000" ;;
        esac
    else
        # 기본 토큰 수를 매우 크게 늘려서 더 많은 비용 발생
        case "$model_name" in
            "claude-3-haiku") echo "100000" ;;  # haiku는 저렴하므로 최대한 많은 토큰
            "claude-3.5-sonnet-v2"|"claude-3.7-sonnet"|"claude-sonnet-4") echo "100000" ;;  # sonnet도 haiku와 동일하게 최대 토큰으로 설정
            *) echo "100000" ;;
        esac
    fi
}
